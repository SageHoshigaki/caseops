...
export default function PacketsPage() {
  ...
  useEffect(() => {
    (async () => {
      const [p, c] = await Promise.all([getPackets(), getClients()]);
      setPackets(p); setClients(c); setLoading(false);
    })();
  }, []);

  async function remove(id) {
    if (!confirm("Delete this packet?")) return;
    setPackets(await deletePacket(id));
  }

  if (loading) {
    return (
      <div className="flex h-full items-center justify-center bg-black text-white">
        <div className="mx-auto h-8 w-8 animate-spin rounded-full border-2 border-[#39FF88] border-t-transparent" />
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full">
      <PageHeader
        ...
        actions={<button onClick={() => navigate("/builder")} className="btn-primary"><FilePlus size={14} /> New Packet</button>}
      />
      <PageBody>
        {packets.length === 0 ? (
          <div className="flex flex-col items-center py-24 text-center">
            ...
            <button onClick={() => navigate("/builder")} className="btn-primary mt-4"><FilePlus size={14} /> Build first packet</button>
          </div>
        ) : (
          <div className="space-y-2">
            {[...packets].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).map(p => {
              ...
              return (
                <div key={p.id} className="card px-5 py-4 flex items-center gap-5 hover:border-ink-200 transition-colors cursor-pointer group" onClick={() => navigate(`/builder/${p.id}`)}>
                  ...
                  <div className="flex items-center gap-1 text-xs text-ink-300 shrink-0"><Clock size={11} />{format(new Date(p.createdAt), "MMM d, yyyy")}</div>
                  <button onClick={e => { e.stopPropagation(); remove(p.id); }} className="w-7 h-7 rounded-lg flex items-center justify-center text-ink-300 hover:bg-rose-50 hover:text-rose-400 opacity-0 group-hover:opacity-100 transition-all"><Trash2 size={13} /></button>
