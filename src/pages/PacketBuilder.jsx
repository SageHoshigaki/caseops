...
export default function PacketBuilder() {
  ...
  const [allAttys, setAllAttys]     = useState([]);
  const [loading, setLoading]       = useState(true);

  // Async data loading
  useEffect(() => {
    (async () => {
      const [c, d, v, aCa, aGa] = await Promise.all([
        getClients(), getDefendants(), getViolations(),
        getAttorneysByDistrict("california"), getAttorneysByDistrict("georgia"),
      ]);
      setClients(c); setDefendants(d); setViolations(v);
      setAllAttys([...aCa, ...aGa]);

      if (id) {
        const pkts = await getPackets();
        const p = pkts.find(x => x.id === id);
        if (p) {
  ...
  const [currentAttorneys, setCurrentAttorneys] = useState([]);

  useEffect(() => {
    if (currentPacket.district) {
      getAttorneysByDistrict(currentPacket.district).then(setCurrentAttorneys);
    } else {
      setCurrentAttorneys([]);
    }
  }, [currentPacket.district]);
    ...
    return (
      <div className="flex h-full items-center justify-center bg-black text-white">
        <div className="text-center">
          ...
          <p className="text-sm text-[#A9B7AF]">Loading packet data…</p>
        </div>
      </div>
    );
  }
