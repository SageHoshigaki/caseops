...
import { getDefendants, saveDefendant, deleteDefendant, bulkImportDefendants } from "../lib/store.js";

const CSV_FIELDS = [
  { key: "name",            label: "Company Name", required: true },
  { key: "type",            label: "Entity Type (LLC/Corp/LP/Individual/Other)" },
...
export default function DefendantsPage() {
  return (
    <LibraryPage
      ...
      loadItems={getDefendants}
      onSave={saveDefendant}
      onDelete={deleteDefendant}
      onBulkImport={bulkImportDefendants}
      emptyIcon={Building2}
      ...
      renderCard={(d) => (
        <div>
          ...
          {d.notes && (
            <div className="mt-2 text-[11px] text-ink-300 line-clamp-2 border-t border-ink-50 pt-2">{d.notes}</div>
          )}
        </div>
      )}
  ...
  return (
    <form id="lib-form" className="space-y-4" onSubmit={e => { e.preventDefault(); onSave(form); }}>
      ...
      <Select label="Entity type" value={form.type} onChange={set("type")}>
        <option>LLC</option><option>Corp</option><option>LP</option><option>Individual</option><option>Other</option>
      </Select>
